...
func (s *Scheduler) doScheduleBinding(namespace, name string) (err error) {
	...
	if rb.Spec.Suspend {
		s.recordScheduleResultEventForResourceBinding(rb, nil, fmt.Errorf("skip schedule suspended resourceBinding: %s/%s", rb.Namespace, rb.Name))
...
func (s *Scheduler) doScheduleClusterBinding(name string) (err error) {
	...
	if crb.Spec.Suspend {
		s.recordScheduleResultEventForClusterResourceBinding(crb, nil, fmt.Errorf("skip schedule suspended resourceBinding: %s/%s", crb.Namespace, crb.Name))
		klog.V(4).InfoS("Skip schedule suspended ResourceBinding", "ResourceBinding", klog.KObj(crb))
		return nil
	}
